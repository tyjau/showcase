...
// a comma decimal (1 234 $ / 1 234 €). Intl.NumberFormat handles both per locale,
// so the displayed format follows the active UI language — not a hardcoded en-US style.
// `narrowSymbol` keeps the compact glyph (USD → "$", not "$US"; EUR → "€").

function bcp47(lang: string): string {
  return lang === "fr" ? "fr-FR" : "en-US";
}

/** Whole-currency amount (totals): no decimals. e.g. en "$1,234" · fr "1 234 $". */
